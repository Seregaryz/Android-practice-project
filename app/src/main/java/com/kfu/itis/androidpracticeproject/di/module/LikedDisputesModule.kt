package com.kfu.itis.androidpracticeproject.di.module

import androidx.lifecycle.ViewModel
import com.kfu.itis.androidpracticeproject.di.vm_provide.ViewModelKey
import com.kfu.itis.androidpracticeproject.view_model.LikedDisputesViewModel
import dagger.Binds
import dagger.Module
import dagger.multibindings.IntoMap

@Module
interface LikedDisputesModule {

    @IntoMap
    @Binds
    @ViewModelKey(LikedDisputesViewModel::class)
    fun bindLikedDisputesViewModel(likedDisputesViewModel: LikedDisputesViewModel): ViewModel

}
